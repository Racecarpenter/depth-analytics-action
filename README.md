...
- **Ledger, not a counter** — `action_credit_transactions` is an append-only table (`starter_grant`, `referral_reward`, `action_pack_purchase`, `action_created`, `admin_adjustment`, `beta_grant`); the current balance is always `sum(amount)`, never a stored/mutable field. This is deliberate, for auditability — you can always reconstruct exactly why a user has the balance they have.
- **Authorization is server-side and race-safe** — every Action creation calls the `consume_action_credit_or_pass` Postgres function (`supabase/migrations/0005_monetization.sql`, updated in `0018_beta_paywall_credits.sql`), which takes an advisory lock per user before checking for an active pass or spending a credit. Two simultaneous requests from a user with exactly one credit cannot both succeed.
...
- **Beta testers hit the real paywall too** — see "Beta paywall credits" below for the temporary, Stripe-free `beta_grant` option that exists specifically so beta testers can be observed going through the real funnel.
...
The root layout (`src/app/layout.tsx`) and `/how-it-works` (`src/app/how-it-works/page.tsx`) both point their `openGraph`/`twitter` metadata at one asset — `public/action-og.png` (1200×650, a static file in `/public`, not a Next.js Image Optimization URL) — via the `SOCIAL_IMAGE_*` constants in `src/lib/constants.ts`. That's the single source of truth on purpose: one asset, one set of dimensions, referenced everywhere a social card shows up, so `og:image` and `twitter:image` can never end up pointing at two different crops.

**The declared width/height must always equal the file's actual pixel dimensions.** Several major crawlers — notably Apple's Link Presentation framework (iMessage) — trust the declared `og:image:width`/`og:image:height` rather than fetching the image to check, and render a small, mostly-blank vertical card instead of the wide "hero" layout when the image is undersized relative to the ~1200×630 (1.91:1) size they expect. That's exactly what happened before this was fixed: the asset was a clean 633×343 export, accurately declared as 633×343 — correct metadata, just an image too small for crawlers to treat as a hero card. The fix was resizing the same, unmodified artwork up to 1200×650 (Lanczos, aspect ratio preserved exactly — no crop, no redesign) and updating the constants to match.

If you ever replace `public/action-og.png`, re-check its actual dimensions (`file public/action-og.png` or Pillow's `Image.open(...).size`) and update `SOCIAL_IMAGE_WIDTH`/`SOCIAL_IMAGE_HEIGHT` in `src/lib/constants.ts` to match exactly — don't just eyeball it or assume the old numbers still apply.
...
-- becomes what they fall back to immediately. Revoking also removes access
-- to the free beta-credit paywall option.
...
The goal is for beta testers to experience the real paywall — reach the wall naturally, see the real pricing UI, understand Action credits — without Stripe processing a real payment. When a beta tester's balance hits zero (and they have no active Pass), the paywall shows an extra "Beta access — 5 Actions — Free" card alongside the real purchase options. Tapping it inserts a genuine `+5` row into `action_credit_transactions` (`type: 'beta_grant'`) through the exact same ledger the eventual Stripe fulfillment will write to — never a fake balance, never a fake `purchases`/Stripe record. It's deliberately repeatable: every time their balance returns to zero, the option comes back, so "use Actions → hit paywall → claim more → continue" can repeat as many times as they want to test it.

**Everything this needs lives in four places**, on purpose, so it's easy to remove once Stripe goes live:
- `credit_transaction_type` value `'beta_grant'` (`0017_beta_grant_credit_type.sql`) — distinguishable from `starter_grant`, `referral_reward`, `action_pack_purchase`, and `admin_adjustment` in the ledger, so `select count(*) from action_credit_transactions where type = 'beta_grant' group by user_id` tells you exactly how many times each beta tester voluntarily chose to keep going after hitting zero — real beta signal.
- `grant_beta_paywall_credits(p_user_id, p_amount)` (`0018_beta_paywall_credits.sql`) — the only place a `beta_grant` row is ever inserted. Re-derives eligibility itself (active `beta_unlimited` entitlement, no active Pass, balance `<= 0`) rather than trusting its caller, same as `consume_action_credit_or_pass`.
- `src/features/monetization/lib/beta-credits.ts` / `beta-mutations.ts` — `isBetaFreeCreditsFeatureEnabled()` reads `ENABLE_BETA_FREE_CREDITS`; `claimBetaPaywallCredits()` is the one Server Action the UI calls, re-checking auth + eligibility server-side regardless of what the client believes.
- `src/features/monetization/components/beta-paywall-credits.tsx` — the one UI card, rendered by `Paywall` only when the server already resolved both `isBetaTester` and the feature flag.

**To turn it off when Stripe goes live**: set `ENABLE_BETA_FREE_CREDITS=false` (or delete it — same effect, see `.env.example`) in Vercel and redeploy. The card disappears from the paywall and `claimBetaPaywallCredits` refuses server-side even if someone calls it directly. Nothing else in the monetization system changes — the normal credit ledger, referral rewards, Action Pass logic, and Stripe checkout are untouched by any of this.

**Production purchase buttons stay Stripe-config-aware, not beta-aware**: Action Pack/Action Pass show a real, working "Buy"/"Get the Pass" button whenever `STRIPE_PRICE_ACTION_PACK`/`STRIPE_PRICE_ACTION_PASS` are set, and a disabled "Coming soon" button otherwise — for every user, not just beta testers. That's intentionally a separate concern from beta status: it's "is Stripe live," and it fixes itself the moment those env vars are set for real, no code change needed.

---

## User profiles

Phone numbers get someone into ACTION; a lightweight profile is who they are once they're there. This is deliberately small — display name, a unique `@username`, and one photo — and it is **not** a social network: no friend requests, no followers, no global user search, no public profiles, no feeds. The only thing you can quickly reuse is people you've already had an Action with.

**Identity resolution never touches historical rows.** `Actions`/`participants` have only ever stored `user_id` + `phone` — never a copied name — so `src/features/users/lib/identity.ts`'s `resolveIdentity()` resolves display name/username/avatar live off the current `users` row every time something renders a participant. Someone shown as `•••-•••-1234` today automatically shows `Mike / @mikey` on every existing Action the moment they complete their profile — no backfill, no `UPDATE` across old rows. Falls back to a masked phone number whenever `display_name` is unset. `ParticipantIdentity` (`src/features/users/components/`) is the one shared component that renders this consistently; leaf components that stayed plain-text (settlement cards, the voting panel, the multi-obligation list) go through the same resolver via a `nameFor`/`identityFor` closure built once per page rather than each re-deriving the fallback.

**Schema** (`0019_user_profiles.sql`): `users.username` (nullable, `^[a-z][a-z0-9_]{2,19}$`, mirrors the existing `cashtag_format` constraint's shape) and `users.avatar_path` (nullable, a Storage object path — never a URL, never base64 in the database). Username uniqueness is enforced at the database level via `create unique index on users (lower(username)) where username is not null` — a plain unique index would still allow `"Mike"` and `"mike"` as two different rows; no `citext` extension needed. App code (`src/lib/validations/profile.ts`) normalizes to lowercase and gives friendly errors before ever hitting the DB, but the index is the actual source of truth — `updateProfile()` translates a `23505` unique-violation into "That username is taken."

**Avatar storage** (`0020_avatar_storage.sql`): a **public** Supabase Storage bucket (`avatars`, 3 MB limit, jpeg/png/webp), unlike the private `custom-action-proof` bucket. Deliberate difference: proof photos are Action-scoped and only meant for that Action's participants, so private + signed URLs is right there. Avatars render across potentially many participants on one page (Home, Action detail) — signed URLs would mean a signing round-trip per avatar, exactly the N+1 pattern this feature was asked to avoid. A public bucket with a deterministic path (`{user_id}/avatar.{ext}`) lets the app build the URL as a pure string (`getAvatarUrl()`) with zero extra network calls. RLS policies still restrict writes to `(storage.foldername(name))[1] = auth.uid()::text` as defense-in-depth, even though uploads only ever go through the server (service-role client), never direct browser-to-storage.

**"People you've had Action with"** is derived, not a Friends table — `get_people_with_action_history` (`0022_relationship_stats.sql`) connects two users only once **both sides have `status = 'accepted'`** on a shared Action; an invite that was only sent, or was declined, never creates a connection. It's the same function powering the recent/frequent list (empty search) and the search box on both creation pickers, ordered by most recent shared Action then by Actions-together count — a simple blended sort, not a scored ranking. **This is the entire privacy boundary for "no global search":** the RPC is `SECURITY DEFINER`, revoked from every API role except `service_role`, and the one client-reachable entry point (`searchPreviousOpponents`, a Server Action) always passes the signed-in caller's own id — there's no parameter a client could use to search anyone else's connections or the wider `users` table.

**Sports/Custom Action creation** (`ActionBuilder`, `CustomActionBuilder`) show the person picker above the existing phone-number path, with an "OR" divider that only appears once there's someone to reuse. Selecting an existing user creates their `participants` row directly (`user_id` pre-filled, `status: 'invited'`) with **no SMS sent** — identity is already established, so they see the invite in-app via the existing "Needs your response" flow from the SMS-independence work; a typed phone number still gets the SMS invite link exactly as before. `inviteParticipant()` (`src/features/actions/lib/invite-participant.ts`) accepts either `phone` or `userId`; `createCustomActionAndInvite` mixes both freely, deduping in both directions (no picking the same person twice, no typing a phone number that belongs to someone already picked).

**Lightweight profile page** (`/players/[userId]`) shows global stats (Wins–Losses, Total Actions, Settlement %) and a head-to-head block ("You vs Mike") — never a mysterious 0–100 score, just the transparent numbers the product spec asked for. Access is gated entirely by shared history: `get_head_to_head_stats` returns `actions_together: 0` for any pair with no genuine (both-sides-accepted) Action together, and the page 404s identically in that case or if the user doesn't exist — no way to distinguish "no history" from "doesn't exist," so there's no unrestricted user directory even by inference. **Settlement rate** is debtor-side only (`settled_count / owed_total_count`, counting only obligations where this user owed money — pushes, cancellations, and obligations owed *to* them are excluded from the denominator), representing "when I owe, do I pay," the classic trust signal, rather than mixing in the other party's reliability. **Head-to-head net amount** sums every direct obligation between exactly two users, in either direction, regardless of `payment_status` — the "if everything settled today" number, with `all_settled` reported separately. Both read straight from `settlement_obligations`, the same table Action Recap/settlement UI already use — no second settlement calculation. The 4-person-Custom-Action pairwise case (Mike beats three people for $20 each → Mike is `+$20` against each of them individually, never `+$80` against one) falls out of the schema for free: `settlement_obligations` already stores one row per debtor→creditor pair, never one row per pot.

**Profile completion notification**: created exactly once, only inside `verifyOtp`'s brand-new-account branch (never on a returning login), type `profile_completion` — the one notification type with no `action_id` (see `notificationHref()`'s type-based special case routing it to `/profile` instead of an Action). No separate `profile_completed_at` column: the one-time creation point already gives natural dedup semantics, and `updateProfile()` marks that notification read the moment a profile is actually saved.

**Manual setup**: none beyond running the new migrations — the avatars bucket and its policies are created by `0020_avatar_storage.sql` itself, no dashboard steps required.

**Intentionally not built** (per the V1.2 scope): friend requests/followers, public discovery or a global user directory, messaging/chat, feeds/comments/likes, public profiles reachable without shared history, a numeric social score. The data is architected so a future "Action Score" could be derived from the same stats later, but nothing computes one today.
